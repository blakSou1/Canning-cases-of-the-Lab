...
namespace Assets.SimpleLocalization.Scripts
{
    ...
    public static class LocalizationManager
    {
        ...
        public static string Localize(string localizationKey, string column)
        {
            if (Dictionary.Count == 0)
                Read();
            ...
            var missed = !Dictionary[column].ContainsKey(localizationKey) || Dictionary[column][localizationKey] == "";

            if (missed)
            {
                Debug.LogWarning($"Translation not found: {localizationKey} ({column}).");
